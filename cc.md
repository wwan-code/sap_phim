...
Hãy tạo hệ thống giao diện **Comment UI** mới theo các yêu cầu sau:

---

## Công nghệ & Kiến trúc

- Frontend - Project: React + Vite (JS/JSX), Redux Toolkit, Redux Thunk/RTK Query optional, SCSS (no Tailwind).
...
### Frontend tham chiếu:
    ...
    @/frontend/src/store/slices/friendSlice.js
...
- Cung cấp ngữ cảnh: Nêu rõ tech stack, kiến trúc Redux, endpoints, state shape, realtime, style SCSS BEM.
- Yêu cầu đầu ra: Component, service, slice Redux, SCSS, và tích hợp vào `MovieDetailPage.jsx` và `MovieWatchPage.jsx` có ví dụ gọi API.
...
- Tạo Redux store với commentSlice chứa:
    - Store chuẩn hóa:
    {
        ...
        entities: {
            byId: { [id]: Comment },
            allIds: string[],
            childrenByParentId: { [parentId]: string[] }
        },
        
        // UI state
        ui: {
            activeSort: 'latest' | 'oldest' | 'popular',
            composingForId: string | null,
            editingId: string | null,
            expandedReplies: Set<string>,
            loadingStates: { [id]: boolean }
        },
        
        // Pagination per content
        pagination: {
            [contentKey]: {
            page: number,
            limit: number,
...
- If comment.hasReplies => show “Show replies” button that triggers lazy load getReplies(parentId) via service.
...
- Hiển thị các trạng thái: empty state (chưa có bình luận), skeleton khi loading, inline error với nút retry, disabled submit khi invalid.
...
---
**Đảm bảo**: UI cập nhật ngay lập tức khi gửi/chỉnh sửa comment. Viết đầy đủ các tính năng.
...
- Nút "Tải thêm" hoạt động theo `meta.page`/`meta.totalPages` cho cả root và replies.
- Tạo/sửa/xóa/like/report hoạt động với optimistic update và rollback khi thất bại.
- Realtime nhận sự kiện và cập nhật UI không flicker, không nhân đôi item.
...
- Dùng selector memo hóa (reselect) cho deriving list theo depth/sort.
- Tách nhỏ component, dùng `React.memo` cho `CommentItem` và `CommentActions`.
- Batch updates khi nhận chùm sự kiện realtime.
...
=> Viết code chi tiết cho toàn bộ các yêu cầu ở trên và tạo đầy đủ các files yêu cầu, đảm bảo chạy được ngay trong dự án React + Vite + Redux Toolkit + SCSS.