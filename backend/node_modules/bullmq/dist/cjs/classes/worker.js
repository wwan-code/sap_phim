        ...
        return this.trace(enums_1.SpanKind.CONSUMER, 'process', this.name, async (span) => {
            ...
            try {
                ...
                if (unrecoverableErrorMessage) {
                    const failed = await this.retryIfFailed(() => this.handleFailed(new errors_1.UnrecoverableError(unrecoverableErrorMessage), job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), this.opts.runRetryDelay);
                    return failed;
                }
                jobsInProgress.add(inProgressItem);
                const result = await this.callProcessJob(job, token);
                return await this.retryIfFailed(() => this.handleCompleted(result, job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), this.opts.runRetryDelay);
            }
            catch (err) {
                const failed = await this.retryIfFailed(() => this.handleFailed(err, job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), this.opts.runRetryDelay);
                return failed;
            }
            finally {
                span === null || span === void 0 ? void 0 : span.setAttributes({
